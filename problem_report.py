...
#         # Ensure the base URL contains a '?' for query parameters
#         if '?' not in base_url:
#             base_url += '?'
#         else:
#             base_url += '&'
#         base_url += 'pageSize=500'  # Add pageSize parameter
...
#         while True:
#             params["page"] = page  # Add page parameter for pagination
...
#                 # If all problems have been fetched, break the loop
...
#         # Process and save the problems as a DataFrame
#         ist = "Asia/Kolkata"
#         problems = [
#             {
...
#                 "Start Time (IST)": pd.to_datetime(problem.get("startTime"), unit='ms').tz_localize("UTC").tz_convert(ist).replace(tzinfo=None),
#                 "End Time (IST)": (
#                     pd.to_datetime(problem.get("endTime"), unit='ms').tz_localize("UTC").tz_convert(ist).replace(tzinfo=None)
#                     if problem.get("endTime") != -1
#                     else "Ongoing"
#                 ),
...
# ttk.Label(frame, text="From Date:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
# from_date_picker = DateEntry(frame, width=20, date_pattern="yyyy-mm-dd")
# from_date_picker.grid(row=1, column=1, pady=5, sticky=tk.W)

# ttk.Label(frame, text="From Time:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
# from_hour_combobox = ttk.Combobox(frame, values=[f"{i:02d}" for i in range(24)], width=5)
# from_hour_combobox.grid(row=2, column=1, sticky=tk.W, padx=(0, 5))
# from_hour_combobox.set("00")
# from_minute_combobox = ttk.Combobox(frame, values=[f"{i:02d}" for i in range(60)], width=5)
# from_minute_combobox.grid(row=2, column=1, padx=(60, 0), sticky=tk.W)
# from_minute_combobox.set("00")

# ttk.Label(frame, text="To Date:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
# to_date_picker = DateEntry(frame, width=20, date_pattern="yyyy-mm-dd")
# to_date_picker.grid(row=3, column=1, pady=5, sticky=tk.W)

# ttk.Label(frame, text="To Time:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
# to_hour_combobox = ttk.Combobox(frame, values=[f"{i:02d}" for i in range(24)], width=5)
# to_hour_combobox.grid(row=4, column=1, sticky=tk.W, padx=(0, 5))
# to_hour_combobox.set("23")
# to_minute_combobox = ttk.Combobox(frame, values=[f"{i:02d}" for i in range(60)], width=5)
# to_minute_combobox.grid(row=4, column=1, padx=(60, 0), sticky=tk.W)
# to_minute_combobox.set("59")

# ttk.Label(frame, text="Management Zone:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
# management_zone_entry = ttk.Entry(frame, width=50)
# management_zone_entry.grid(row=5, column=1, pady=5)

# ttk.Label(frame, text="API Token:").grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
# token_entry = ttk.Entry(frame, width=50, show="*")
# token_entry.grid(row=6, column=1, pady=5)

# # Fetch Button
# ttk.Button(frame, text="Fetch Data", command=fetch_data).grid(row=7, column=0, columnspan=2, pady=20)
